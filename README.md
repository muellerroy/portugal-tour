...
## Update: Stabilität & Usability (Version 3)

**Behobene Fehler:**
- **Karten-Zoom-Bug:** Die Karte startete je nach Timing auf maximaler Zoomstufe statt der
  Portugal-Übersicht (fitBounds lief, während der Karten-Screen noch unsichtbar war). Behoben.
- **Fotospeicher:** Fotos werden jetzt vor dem Speichern automatisch verkleinert (max. 1280 px,
  JPEG). Vorher liefen die ~5 MB localStorage nach 2-3 Originalfotos voll und Speichern schlug
  still fehl. Zusätzlich warnt die App jetzt sichtbar, falls der Speicher doch voll läuft.
- **Fotolöschung:** Ein Tipp aufs Miniaturbild löschte das Foto sofort und ohne Rückfrage.
  Jetzt öffnet sich eine Grossansicht mit separatem Löschen-Button und Bestätigung.

**Neu:**
- **Daten importieren:** Das JSON-Backup lässt sich in den Einstellungen wieder einspielen
  (auch auf einem anderen Gerät). Export-Dateien tragen jetzt das Datum im Namen.
- **Speicheranzeige:** Die Einstellungen zeigen den belegten Speicher an und warnen ab 80%.
- **Update-Banner:** Wird die App neu deployt (z. B. neue Spots in data.js), erkennt die
  installierte App das im Hintergrund und bietet einen Neustart an. Vorher blieben Updates
  wegen Cache-first dauerhaft unsichtbar, bis man den Cache manuell leerte.
- **Leaflet lokal gebündelt** (vendor/leaflet/): Die Karte funktioniert jetzt auch dann,
  wenn die App nach der Installation zum ersten Mal offline geöffnet wird. Vorher hing sie
  am unpkg-CDN.
- **Notizen speichern automatisch** beim Tippen (mit "Gespeichert"-Hinweis), nicht mehr erst
  beim Verlassen des Felds.
- **Tagebuch-Zähler** am Tab, Feedback-Toasts (Koordinaten kopiert, Backup geladen, importiert),
  Wetteranzeige nennt das echte Alter der Cache-Daten ("vor 2 Std.").
- **Bedienbarkeit:** Zoom-Sperre entfernt, Eingabefelder auf 16 px (kein iOS-Auto-Zoom mehr),
  Escape schliesst Sheet und Grossansicht, Screenreader-Labels für Kartenfilter, Sterne und Fotos,
  Notizen werden HTML-sicher dargestellt.
- Die App bittet den Browser um dauerhafte Speicherung (navigator.storage.persist), damit
  iOS die lokalen Daten nicht ungefragt aufräumt. Der Wetter-Cache räumt sich selbst auf
  (Einträge älter als 7 Tage).

## Update: Redesign "Geovea-Edition" (Version 4)

Komplettes visuelles Redesign nach Vorbild der Geovea-App (Behance/Ron Design Lab),
Funktionen unverändert:
- **Typografie:** Urbanist (wie Geovea), klare Gewichte, Uppercase-Mikrolabels
- **Palette:** minimal – fast weisse Flächen, Hairlines statt Schatten, eine dunkle
  Kontrastfläche (Hero + Navigation) und **Violett (#7C5CE0) als einzige Akzentfarbe**
- **Navigation:** dunkle, schwebende Pill-Navigation mit Linien-Icons statt Emojis
- **Karte:** Full-Bleed hinter der Navigation, harmonisierte Kategoriefarben
  (Camping jetzt Petrol statt Violett, damit der Akzent frei bleibt)
- **Azulejo dezent integriert:** feine Kachelbordüre unter der Kopfzeile und
  Azulejo-Wasserzeichen in der dunklen Hero-Karte (7% Deckkraft)
- **Hahn von Barcelos** neu eingefärbt: schwarzvioletter Körper mit violett-goldenen
  Schwanzfedern (näher am traditionellen Original, passend zur Palette)
- **App-Icons** neu im dunklen Look mit violettem Pin
